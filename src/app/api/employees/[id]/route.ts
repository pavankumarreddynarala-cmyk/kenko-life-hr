...
// Soft delete only — the record is never removed from the database. It is excluded
// from the default Employee Master listing and can be brought back with the restore
// endpoint (see [id]/restore/route.ts).
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const employee = await db.$transaction(async (tx) => {
      const before = await tx.employee.findUnique({ where: { id }, include: employeeInclude });
      if (!before) throw new Error("Employee not found");
      if (before.deletedAt) throw new Error("Employee already deleted");
      const after = await tx.employee.update({ where: { id }, data: { deletedAt: new Date() } });
      await tx.employeeHistory.create({
        data: { employeeId: id, snapshot: before as never, reason: "Employee deleted (soft delete)" },
      });
      await audit(
        {
          actorId: session.userId,
