...
export async function GET(req: NextRequest) {
  try {
    const session = requireRole(req, ["ADMIN", "HR", "CFO"]);
    ...
    if (type === "employees") {
      ...
      if (session.role !== "CFO" && session.role !== "ADMIN") {
        return NextResponse.json({ error: "CFO access required for asset export" }, { status: 403 });
      }
      ...
      rows = [{ "Asset ID": "", "Asset Category": "", "Asset Description": "", "Serial No.": "", "Purchase Cost": "", "Useful Life": "", "Depreciation Method": "" }];
    } else {
