...
export default function EmployeePortalPage() {
  ...
  const [stage, setStage] = useState<"email" | "otp" | "onboarding" | "portal">("email");
  ...
  async function send() {
    ...
    setBusy("send");
    try {
      const body = await requestJson<{ message: string; email: string }>("/api/otp", {
        method: "POST",
  ...
  async function verify() {
    ...
    setBusy("verify");
    try {
      const body = await requestJson<{ isNew: boolean; email: string }>("/api/otp/verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        <div className="card mx-auto mt-12 max-w-2xl">
          ...
          {stage === "email" && (
            <div className="mt-6 space-y-3">
              ...
              <input autoComplete="email" className="input" inputMode="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@company.com" />
              <p className="text-xs text-stone-500">We&apos;ll email a one-time code to verify it&apos;s you.</p>
          ...
          {stage === "otp" && (
            <div className="mt-6 space-y-3">
              ...
              <button className="w-full text-sm text-stone-500 underline" onClick={() => setStage("email")}>Use another email</button>
            </div>
          )}
          {stage === "onboarding" && (
      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-6 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-stone-500">{employee?.permanentId} · {employee?.email}</p>
          </div>
