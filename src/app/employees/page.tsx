...
function EmployeesPageInner() {
  ...
  const load = useCallback(async (deletedView: boolean) => {
    const [employeeResponse, masterResponse] = await Promise.all([
      fetch(`/api/employees${deletedView ? "?deleted=1" : ""}`, { cache: "no-store" }),
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setDeleting(null);
      setNotice(
        `${deleting.name} (${deleting.permanentId}) was deleted. ` +
          "You can restore them any time from \u201cShow deleted employees\u201d below.",
      );
      await load(showDeleted);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Unable to delete employee");
    } finally {
      setBusyId(null);
    }
  }

  const restore = useCallback(async (row: Employee) => {
    setBusyId(row.id);
    try {
      const response = await fetch(`/api/employees/${row.id}/restore`, { method: "POST" });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? "Unable to restore employee");
      setNotice(`${row.name} (${row.permanentId}) was restored.`);
      await load(showDeleted);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Unable to restore employee");
    } finally {
      setBusyId(null);
    }
  }, [load, showDeleted]);
  ...
  return (
    <Shell>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <button className="btn-primary" onClick={() => setEditing({ pfEligible: false, status: "ACTIVE" })} disabled={showDeleted}>
          + Add employee
        </button>
      ...
      <div className="card overflow-hidden p-0">
        <FilterableTable
          rows={rows}
          columns={columns}
          emptyMessage={showDeleted ? "No deleted employees." : "No employees found."}
        />
      </div>
      {notice && <p className="mt-4 rounded-lg bg-orange-50 p-3 text-sm text-orange-900">{notice}</p>}
      {deleting && (
        <div className="fixed inset-0 z-50 bg-black/40 p-4">
          <section className="mx-auto mt-24 max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <h3 className="text-lg font-bold">Delete employee?</h3>
            <p className="mt-2 text-sm text-stone-600">
              You are about to delete <strong>{deleting.name}</strong> ({deleting.permanentId}). This can be undone
              from &ldquo;Show deleted employees&rdquo; below.
            </p>
            <div className="mt-5 flex justify-end gap-3">
              ...
              >
                {busyId === deleting.id ? "Deleting…" : "Delete employee"}
              </button>
            </div>
          </section>
        </div>
